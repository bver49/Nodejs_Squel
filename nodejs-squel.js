...
function handleDisconnect() {
	connection = mysql.createConnection(db.config);
...
function run(callback) {
  var sqlobj = this.sqlobj;
  switch(sqlobj.sqlType) {
    case 1:
      if(sqlobj.joinTable != "") {
        sqlobj.sql = JoinQueryBuilder(sqlobj);
      } else {
        sqlobj.sql = SelectQueryBuilder(sqlobj);
      }
      break;
    case 2:
      sqlobj.sql = InsertQueryBuilder(sqlobj);
      break;
    case 3:
      sqlobj.sql = DeleteQueryBuilder(sqlobj);
      break;
    case 4:
      sqlobj.sql = UpdateQueryBuilder(sqlobj);
      break;
    default:
      console.log("Error");
      break;
